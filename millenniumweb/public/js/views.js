erpnext.ProductView =  class {
	...
	*/
	...
	prepare_toolbar() {
		...
		frappe.require('/assets/js/millennium-products-listening.min.js', function() {
	...
	prepare_product_area_wrapper(view) {
		...
		return this.products_section.append(`
			<br>
			<div id="products-${view}-area" class="products-list shift finbyz row ${ top_margin } ${ left_margin }"></div>
		`);
	}
	...
	add_paging_section(settings) {
		...
		if($("body").attr("data-path") == 'all-products'){
			if (this.products) {
				let paging_html = `
					<div class="product-paging-area mt-5">
						<div class="col-3">
						</div>
				...
				paging_html += `
					<button class="btn btn-outline-secondary btn-next" data-start="${ start - page_length }"
	...
	render_view_toggler() {
		...
		["btn-list-view", "btn-grid-view"].forEach(view => {
			...
			$(".toggle-container").append(`
				<div class="form-group mb-0 test" id="toggle-view">
						<button id="${ icon }" class="btn ${ view }  mr-2">	
						<img ${view == "btn-list-view" ? `src="/files/list.png"` :  `src="/files/menu.png"`}  alt="address_icon"/>									
				</button>				
				</div>
	...
	render_item_sub_categories(categories) {
		if (categories && categories.length) {a
			let sub_group_html = `
