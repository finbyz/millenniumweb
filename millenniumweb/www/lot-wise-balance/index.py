...
def get_context(context):
    ...
    if 'filters' in context:
        ...
        data = execute()
    context.available_filters = get_filters()
    context.data = data


def get_filters():
    default_filters = ["Item Group",
                       "Tile Quality", "Company"]
    return [{filter: frappe.get_list(filter, fields=['name'])} for filter in default_filters]
...
def execute(filters=None):
    ...
    if not filters:
        filters = {"company": "Millennium Vitrified Tiles Pvt. Ltd. Testing", "from_date": "2020-06-11",
                   "to_date": "2020-06-11", "item_group": "", "tile_quality": "", "item_code": "", "print_with_picked_quantity": 0}
    ...
    for i in range(len(data)):
        if i == 0 or data[i]['tile_quality'] != data[i-1]['tile_quality'] or data[i]['item_design'] != data[i-1]['item_design']:
            data[i].update({'total': data[i]['remaining_qty']})
        else:
            data[i].update(
                {'total': data[i-1]['total'] + data[i]['remaining_qty']})
...
def get_picked_qty(filters, float_precision):
    ...
    for d in picked:
        ...
        picked_dict.pickedqty = flt(
            picked_dict.pickedqty, float_precision) + flt(d.pickedqty, float_precision)
